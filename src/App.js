...
function App() {
  ...
  const parseAndOptimizeWithAngles = (input, tone, angle, kampagnetype, saeson) => {
    let cleanedInput = input.replace(/\([^)]*\)/g, '').replace(/\s+/g, ' ').trim();

    const seasonContext = {
      'Forår': { mood: 'fornyelse og vækst', benefits: 'genoplivende forårsfølelse' },
      'Sommer': { mood: 'varme og livslyst', benefits: 'sommerens magi' },
      'Efterår': { mood: 'hygge og samvær', benefits: 'efterårets ro' },
      'Vinter': { mood: 'varme og nærvær', benefits: 'vinterens tryghed' },
      'Helligdage': { mood: 'fejring og samvær', benefits: 'uforglemmelig fejring' },
      'Særlige begivenheder': { mood: 'fest og glæde', benefits: 'perfekt fejring' }
    };

    const campaignFocus = {
      'Salg': { cta: 'reserver dit ophold' },
      'Trafik': { cta: 'læs mere om oplevelsen' },
      'Engagement': { cta: 'fortæl os om din drøm' },
      'Brand Awareness': { cta: 'lær Hotel Oasia at kende' }
    };
    ...
    const angleTemplates = {
      'Pain': {
        starters: [`Træt af at ${saeson.toLowerCase()} glider forbi uden ægte afslapning?`, `Savner du det perfekte sted til ${currentSeason.mood}?`],
        benefits: [`Hotel Oasia skaber det perfekte ${saeson.toLowerCase()}-pusterum.`, `${currentCampaign.cta} og oplev ${currentSeason.benefits}.`]
      },
      'Outcome': {
        starters: [`Oplev ${currentSeason.benefits} på Hotel Oasia.`, `Find din indre ro gennem ${currentSeason.mood}.`],
        benefits: [`Hver detalje er tænkt med ${saeson.toLowerCase()} i tankerne.`, `${currentCampaign.cta} og mærk forskellen.`]
      },
      'Proof': {
        starters: [`Hundredvis af gæster har fundet deres perfekte ${saeson.toLowerCase()}-ophold hos os.`, `Hotel Oasia er blevet det foretrukne ${saeson.toLowerCase()}-refugium.`],
        benefits: [`Dokumenteret gæstfrihed i alle sæsoner.`, `${currentCampaign.cta} og bliv en del af familien.`]
      },
      'Objection': {
        starters: [`"For dyrt for et ${saeson.toLowerCase()}-ophold" - indtil du ser værdien.`, `"Alle hoteller er ens" - selv i ${saeson.toLowerCase()}?`],
        benefits: [`Værdi der rækker langt ud over ${saeson.toLowerCase()}-sæsonen.`, `${currentCampaign.cta} og prioritér dig selv.`]
      },
      'Urgency': {
        starters: [`Kun få ${saeson.toLowerCase()}-ophold tilbage i denne periode.`, `Sidste chance for det perfekte ${saeson.toLowerCase()}-ophold.`],
        benefits: [`${currentCampaign.cta} før det er for sent.`, `Sikr dig ${currentSeason.benefits} nu.`]
      }
    };
  ...
  const generateAdTexts = async () => {
    ...
    for (let i = 0; i < formData.antal_varianter; i++) {
      ...
      const parsed = parseAndOptimizeWithAngles(formData.input_text, formData.tone, angle, formData.kampagnetype, formData.saeson);

      let primaryText = `${parsed.starter} ${parsed.benefit}`;
      const headline = `${formData.saeson}-ophold - Hotel Oasia`;
      const description = `${parsed.campaignFocus.cta}`;
      ...
      const fullText = `${primaryText} ${headline} ${description}`;
    ...
    const sortedIndices = scores.map((score, index) => ({ score: score.total, index })).sort((a, b) => b.score - a.score).map(item => item.index);
      ...
      <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          ...
          <div className="lg:w-1/3 p-6 bg-gray-50 border-r">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select name="kampagnetype" value={formData.kampagnetype} onChange={handleInputChange} className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
                  {kampagnetyper.map(type => (<option key={type} value={type}>{type}</option>))}
                </select>
              </div>

              <div>
                <label className="block text-sm font-medium mb-2">Sæson/Periode</label>
                <select name="saeson" value={formData.saeson} onChange={handleInputChange} className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
                  {sæsoner.map(saeson => (<option key={saeson} value={saeson}>{saeson}</option>))}
                </select>
              </div>

              <div>
                <label className="block text-sm font-medium mb-2">Tone of Voice</label>
                <select name="tone" value={formData.tone} onChange={handleInputChange} className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
                  {tones.map(tone => (<option key={tone.value} value={tone.value}>{tone.label}</option>))}
                </select>
                {selectedTone && (<p className="text-xs text-gray-600 mt-1">{selectedTone.desc}</p>)}
              </div>

              <div>
                <label className="block text-sm font-medium mb-2">Antal Varianter</label>
                <select name="antal_varianter" value={formData.antal_varianter} onChange={handleInputChange} className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
              ...
              <button onClick={generateAdTexts} disabled={isGenerating || !formData.input_text.trim()} className="w-full bg-gradient-to-r from-amber-600 to-orange-600 text-white p-3 rounded-lg font-semibold hover:from-amber-700 hover:to-orange-700 disabled:opacity-50 flex items-center justify-center">
                {isGenerating ? (<RefreshCw className="animate-spin mr-2 w-5 h-5" />) : null}
          ...
          <div className="lg:w-2/3 p-6">
            <div className="flex justify-between items-center mb-6">
              ...
              {generatedAds.length > 0 && (
                <button onClick={exportToCSV} className="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 flex items-center">
                  <Download className="mr-2 w-4 h-4" />Eksporter CSV
                </button>
              )}
            </div>
                ...
                {generatedAds.map((ad, idx) => (
                  <div key={ad.id} className="bg-white border rounded-xl p-6 shadow-sm">
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="text-lg font-semibold text-amber-600">Variant {ad.id} - {ad.angle}</h3>
                      {qualityScores[idx] && (
                    ...
                    <div className="space-y-4">
                      <div>
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className={`text-xs px-2 py-1 rounded ${ad.primaryTextLength <= 125 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{ad.primaryTextLength}/125 tegn</span>
                        </div>
                        <div className="relative">
                          <textarea value={ad.primaryText} readOnly className="w-full p-3 border rounded-lg bg-gray-50 resize-none" rows={3} />
                          <button onClick={() => copyToClipboard(ad.primaryText)} className="absolute top-2 right-2 p-2 text-gray-400 hover:text-gray-600">
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className={`text-xs px-2 py-1 rounded ${ad.headlineLength <= 40 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{ad.headlineLength}/40 tegn</span>
                        </div>
                        <div className="relative">
                          <input value={ad.headline} readOnly className="w-full p-3 border rounded-lg bg-gray-50" />
                          <button onClick={() => copyToClipboard(ad.headline)} className="absolute top-2 right-2 p-2 text-gray-400 hover:text-gray-600">
                            <Copy className="w-4 h-4" />
                          </button>
                        </div>
                      </div>
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className={`text-xs px-2 py-1 rounded ${ad.descriptionLength <= 20 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{ad.descriptionLength}/20 tegn</span>
                        </div>
                        <div className="relative">
                          <input value={ad.description} readOnly className="w-full p-3 border rounded-lg bg-gray-50" />
                          <button onClick={() => copyToClipboard(ad.description)} className="absolute top-2 right-2 p-2 text-gray-400 hover:text-gray-600">
                            <Copy className="w-4 h-4" />
                          </button>
                        </div>
                      </div>
        ...
        <div className="bg-gray-50 p-6 border-t">
          ...
          <div className="grid md:grid-cols-3 gap-4 text-sm text-gray-700">
            <div><strong>Brand Voice:</strong> Varm og professionel, personlig og imødekommende, indbydende og betænksom</div>
            <div><strong>Sæson Integration:</strong> Automatisk tilpasning til valgte sæson med passende stemning og aktiviteter</div>
            <div><strong>Meta Format:</strong> Primary Text (125), Headline (40), Description (20 tegn) - optimeret til Facebook/Instagram</div>
          </div>
        </div>
      </div>
    </div>
